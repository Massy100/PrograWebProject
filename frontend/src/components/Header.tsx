'use client';

import { useState } from 'react';
import '../styles/header.css';

type HeaderProps = {
    isLoggedIn: boolean;
    marketOpen: boolean;
    totalAmount?: number;
};

export default function Header({ isLoggedIn, marketOpen, totalAmount = 0}: HeaderProps) {
    const [q, setQ] = useState('');
    const [showTotal, setShowTotal] = useState(false);

    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        console.log('Buscar:', q);
    };

    return (
        <header className="header">
            {/* the user acount */}
            <div className="header-userRegister">
              <span className="header-userIcon"><svg stroke="currentColor" fill="currentColor" stroke-width="0" viewBox="0 0 16 16" height="1em" width="1em" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M14 1H2a1 1 0 00-1 1v12a1 1 0 001 1h12a1 1 0 001-1V2a1 1 0 00-1-1zM2 0a2 2 0 00-2 2v12a2 2 0 002 2h12a2 2 0 002-2V2a2 2 0 00-2-2H2z" clip-rule="evenodd"></path><path fill-rule="evenodd" d="M2 15v-1c0-1 1-4 6-4s6 3 6 4v1H2zm6-6a3 3 0 100-6 3 3 0 000 6z" clip-rule="evenodd"></path></svg></span>
            </div>


            {/* the searcher part */}
            <form className="header-searchForm" onSubmit={handleSubmit}>
                <span className="header-searchIcon"><svg stroke="currentColor" fill="currentColor" stroke-width="0" viewBox="0 0 1024 1024" height="1em" width="1em" xmlns="http://www.w3.org/2000/svg"><path d="M909.6 854.5L649.9 594.8C690.2 542.7 712 479 712 412c0-80.2-31.3-155.4-87.9-212.1-56.6-56.7-132-87.9-212.1-87.9s-155.5 31.3-212.1 87.9C143.2 256.5 112 331.8 112 412c0 80.1 31.3 155.5 87.9 212.1C256.5 680.8 331.8 712 412 712c67 0 130.6-21.8 182.7-62l259.7 259.6a8.2 8.2 0 0 0 11.6 0l43.6-43.5a8.2 8.2 0 0 0 0-11.6zM570.4 570.4C528 612.7 471.8 636 412 636s-116-23.3-158.4-65.6C211.3 528 188 471.8 188 412s23.3-116.1 65.6-158.4C296 211.3 352.2 188 412 188s116.1 23.2 158.4 65.6S636 352.2 636 412s-23.3 116.1-65.6 158.4z"></path></svg></span>
                <input
                className="header-searchInput"
                placeholder="Buscar"
                value={q}
                onChange={(e) => setQ(e.target.value)}
                />
            </form>

            {/* buttons for the market and the amount of money */}
            <div className="header-rightSection">
                <div
                className={`header-marketStatus ${
                    marketOpen ? 'header-marketStatus--open' : 'header-marketStatus--closed'
                }`}
                >
                <span className="header-marketDot" />
                <span className="header-marketText">
                    Mercado {marketOpen ? 'Abierto' : 'Cerrado'}
                </span>
                </div>

                {isLoggedIn && (
                <div className="header-totalContainer">
                    <button
                    type="button"
                    className="header-totalButton"
                    onClick={() => setShowTotal((v) => !v)}
                    >
                    Total
                    </button>
                    {showTotal && (
                    <div className="header-totalPopover">
                        <strong>Total: </strong> Q.{totalAmount}
                    </div>
                    )}
                </div>
                )}
            </div>
        </header>
    );
}


