...
class PortfolioService:
    ...
    def get_portfolio_summary(portfolio):
        ...
        summary = {
            'total_investment': portfolio.total_inversion,
            'current_value': portfolio.current_value,
            'total_gain_loss': portfolio.current_value - portfolio.total_inversion,
            'total_gain_loss_percentage': 0,
            'investment_count': investments.count(),
            'stocks_count': investments.values('stock').distinct().count()
        }
        
        if portfolio.total_inversion > 0:
            summary['total_gain_loss_percentage'] = (
                (portfolio.current_value - portfolio.total_inversion) / portfolio.total_inversion
            ) * 100
